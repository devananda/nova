...
class BareMetalPXETestCase(bm_db_base.BMDBTestCase):
    ...
    # ====================================
    # Test functions defined outside class

    def test_random_alnum(self):
        s = pxe.random_alnum(10)
        self.assertEqual(len(s), 10)
        s = pxe.random_alnum(100)
    ...
    def test_build_pxe_config(self):
        ...
        # check header
        self.assertEqual(config[:14], 'default deploy')
        # check deploy kernel
        x = config.index('kernel')
        y = config.index('\n', x)
        self.assertEqual(config[x:y], 'kernel ddd')
        # check deploy ramdisk
        x = config.index('initrd=')
        y = config.index(' ', x)
        self.assertEqual(config[x:y], 'initrd=eee')
        # check deployment id
        x = config.index('deployment_id=', x)
        y = config.index(' ', x)
        self.assertEqual(config[x:y], 'deployment_id=aaa')
        # check deployment key
        x = config.index('deployment_key=', x)
        y = config.index(' ', x)
        self.assertEqual(config[x:y], 'deployment_key=bbb')
        # check boot kernel
        z = config.index('label boot\n')
        x = config.index('kernel', z)
        y = config.index('\n', x)
        self.assertEqual(config[x:y], 'kernel fff')
        # check boot ramdisk
        x = config.index('initrd=', z)
        y = config.index(' ', x)
        self.assertEqual(config[x:y], 'initrd=ggg')
    ...
    def test_build_network_config_dhcp(self):
        self.flags(
                net_config_template=
                    '$pybasedir/nova/virt/baremetal/net-dhcp.template',
                group='baremetal',
            )
    ...
    def test_build_network_config_static(self):
        self.flags(
                net_config_template=
                    '$pybasedir/nova/virt/baremetal/net-static.template',
                group='baremetal',
            )
        net = utils.get_test_network_info()
        net[0][1]['ips'][0]['ip'] = '1.2.3.4'
    ...
    def test_get_tftp_image_info(self):
        ...
        self.assertEqual(res['kernel'][0], 'aaaa')
        self.assertEqual(res['ramdisk'][0], 'bbbb')
        self.assertEqual(res['deploy_kernel'][0], 'cccc')
        self.assertEqual(res['deploy_ramdisk'][0], 'dddd')

        # test override defaults with instance values
        extra_specs = {
    ...
    def test_cache_images(self):
        ...
        self.driver._inject_into_image(self.context, self.instance,
                self.node, self.test_network_info, None, '')
        self.mox.ReplayAll()

        self.driver.cache_images(self.context,
                image_meta=[],
                instance=self.instance,
                node=self.node,
                network_info=self.test_network_info,
    ...
    def test_activate_bootloader(self):
        ...
        self.mox.StubOutWithMock(pxe, 'random_alnum')
        ...
        pxe.random_alnum(32).AndReturn('alnum')
