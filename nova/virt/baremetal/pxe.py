...
pxe_opts = [
    ...
    cfg.StrOpt('net_config_template',
               default='$pybasedir/nova/virt/baremetal/net-dhcp.template',
...
def build_pxe_config(deployment_id, deployment_key, deployment_iscsi_iqn,
    ...
    to the two phases of of booting. This may be extended later.
...
def get_tftp_image_info(instance):
    ...
    for uuid, path in [image_info[label] for label in image_info]:
        if uuid is None:
            raise exception.NovaException(_("Can not activate PXE bootloader, "
                                            "%s is undefined") % label)
...
class PXE(base.NodeDriver):
    ...
    def _cache_image(self, context, instance, image_meta):
        ...
        In a future revision, this functionality will be replaced by a more
        scalable approach: the deployment ramdisk will fetch from Glance
        directly, and write its own last-mile configuration.
    ...
    def _inject_into_image(self, context, instance, node, network_info,
    ...
    def cache_images(self, context, image_meta, instance, node, network_info,
        ...
        self._inject_into_image(context, instance, node, network_info,
                injected_files, admin_password)
    ...
    def activate_bootloader(self, context, node, instance):
        ...
        deployment_key = random_alnum(32)
